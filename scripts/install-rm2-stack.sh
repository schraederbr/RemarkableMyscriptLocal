#!/usr/bin/env bash
# Thin wrapper: prefer the PowerShell installer on Windows hosts.
# On macOS/Linux, drives the same on-device script over ssh.
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
HOST="${1:-10.11.99.1}"
USER="${2:-root}"
echo "==> sync scripts to $USER@$HOST"
ssh "$USER@$HOST" 'mkdir -p /home/root/hwr/scripts /home/root/hwr/bin /home/root/hwr/conf /home/root/hwr/out'
